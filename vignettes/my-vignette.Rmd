...
The `plot_cell` function allows the user to visualize the chosen cellular map with the default colors, based on the *data.tables* containing the graphical information (coordinates and colors for the cellular organelles). The user can specify the character strings "cell", "neuron", "microglia", or "fibroblast" to choose between a generic cellular pictograph or a neuron pictograph. The available_pictograms() function can be used to get an overview on the different types of pictographs offered by our tool. 
...
```{r eval=TRUE, fig.height=4.5, fig.width=10, message=FALSE, warning=FALSE}
